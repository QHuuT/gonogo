...
@click.group()
@click.option('-c', '--config', 'config_path', help='Path to RTM automation configuration file')
...
@click.pass_context
def validate(ctx, rtm_file_path: Optional[str], output_format: str, output: Optional[str]):
    """Validate RTM links."""
    ...
    if output_format == 'json':
        ...
        output_data = {
            ...
            'health_score': (result.valid_links / result.total_links * 100) if result.total_links > 0 else 0
        }
...
@click.option('--copy', is_flag=True, help='Copy result to clipboard (requires pyperclip)')
@click.pass_context
...
@click.option('--rtm-path', default='docs/traceability/requirements-matrix.md', help='RTM file path for relative link calculation')
@click.option('--copy', is_flag=True, help='Copy result to clipboard (requires pyperclip)')
@click.pass_context
def generate_bdd_link(ctx, feature_file: str, scenario_name: str, rtm_path: str, copy: bool):
...
def doctor(ctx):
    ...
    if result.total_links == 0:
        ...
        click.echo(f"{status} RTM Health: {health_score:.1f}% ({result.valid_links}/{result.total_links} links valid)")
