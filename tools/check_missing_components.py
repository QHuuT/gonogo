...
def check_missing_components():
    ...
    for us in us_missing:
        epic_info = (
            f"Epic: {us.epic.epic_id} (component: {us.epic.component})"
            if us.epic
            else "No Epic"
        )
    ...
    def_missing = session.query(Defect).filter(Defect.component.is_(None)).all()

    for defect in def_missing:
        epic_info = (
            f"Epic: {defect.epic.epic_id} (component: {defect.epic.component})"
            if defect.epic
            else "No Epic"
        )
    ...
    test_missing = session.query(Test).filter(Test.component.is_(None)).all()

    # Sample first 10
    for test in test_missing[:10]:
        epic_info = (
            f"Epic: {test.epic.epic_id} (component: {test.epic.component})"
            if test.epic
            else "No Epic"
        )
