...
def fix_rtm_comprehensive():
    ...
    fixes = [
        ...
        ('if exclude_component: exclude_components =', 'if exclude_component:\n        exclude_components ='),
        ('if not epic: raise HTTPException', 'if not epic:\n        raise HTTPException'),
        ('if not test: raise HTTPException', 'if not test:\n        raise HTTPException'),

        # Fix broken for loops
        ('component_filters = [] for comp in components:', 'component_filters = []\n        for comp in components:'),
        ...
        ('"timestamp": datetime.now(datetime.UTC).isoformat(), "epic_status":', '"timestamp": datetime.now(datetime.UTC).isoformat(),\n        "epic_status":'),
        ('"persona": persona.upper(), "metrics":', '"persona": persona.upper(),\n            "metrics":'),
    ]
