...
def extract_dependencies(issue_body: str) -> tuple:
    ...
    # Find blocks
    blocks_patterns = [r"Blocks[:\s]*#(\d+)", r"Blocking[:\s]*#(\d+)"]
...
def sync_user_story_comprehensive(
    session, issue, epic_mapping, parsed_labels, us_id, dry_run=True
):
    ...
    affects_gdpr, gdpr_considerations = extract_gdpr_info(
        issue.get("body", ""), parsed_labels["raw_labels"]
    )
    ...
    if existing:
        ...
        if existing.github_issue_state != issue["state"]:
            updates.append(
                f"github_issue_state: {existing.github_issue_state} -> {issue['state']}"
            )
            if not dry_run:
                existing.github_issue_state = issue["state"]

        if existing.github_labels != parsed_labels["raw_labels_json"]:
            updates.append("github_labels: updated")
            if not dry_run:
                existing.github_labels = parsed_labels["raw_labels_json"]

        if existing.github_assignees != json.dumps(assignees):
            updates.append("github_assignees: updated")
            if not dry_run:
                existing.github_assignees = json.dumps(assignees)
        ...
        if existing.acceptance_criteria != acceptance_criteria:
            updates.append("acceptance_criteria: updated")
            if not dry_run:
                existing.acceptance_criteria = acceptance_criteria

        # Update business value if current is empty or new value is better
        if business_value and (
            not existing.business_value
            or existing.business_value in ["TBD", "[Business value to be defined]"]
        ):
            updates.append("business_value: populated")
            if not dry_run:
                existing.business_value = business_value
        elif existing.business_value != business_value:
            updates.append("business_value: updated")
            if not dry_run:
                existing.business_value = business_value

        if existing.priority != (parsed_labels["priority"] or "medium"):
            updates.append(
                f"priority: {existing.priority} -> {parsed_labels['priority'] or 'medium'}"
            )
            if not dry_run:
                existing.priority = parsed_labels["priority"] or "medium"
        ...
        if existing.implementation_status != implementation_status:
            updates.append(
                f"implementation_status: {existing.implementation_status} -> {implementation_status}"
            )
            if not dry_run:
        ...
        if existing.affects_gdpr != affects_gdpr:
            ...
            if not dry_run:
                existing.affects_gdpr = affects_gdpr

        if existing.gdpr_considerations != gdpr_considerations:
            updates.append("gdpr_considerations: updated")
            if not dry_run:
                existing.gdpr_considerations = gdpr_considerations

        if existing.target_release_version != parsed_labels["release"]:
            updates.append(
                f"target_release_version: {existing.target_release_version} -> {parsed_labels['release']}"
            )
            if not dry_run:
                existing.target_release_version = parsed_labels["release"]

        if existing.sprint != parsed_labels["sprint"]:
            updates.append(f"sprint: {existing.sprint} -> {parsed_labels['sprint']}")
            if not dry_run:
                existing.sprint = parsed_labels["sprint"]

        if existing.depends_on_issues != json.dumps(depends_on):
            updates.append("depends_on_issues: updated")
            if not dry_run:
                existing.depends_on_issues = json.dumps(depends_on)

        if existing.blocks_issues != json.dumps(blocks):
            updates.append("blocks_issues: updated")
            if not dry_run:
                existing.blocks_issues = json.dumps(blocks)

        if existing.github_issue_url != issue["html_url"]:
            updates.append("github_issue_url: updated")
            if not dry_run:
                existing.github_issue_url = issue["html_url"]
...
def sync_defect_comprehensive(
    session, issue, epic_mapping, parsed_labels, def_id, dry_run=True
):
    ...
    if existing:
        ...
        if gdpr_considerations and not existing.gdpr_impact_details:
            updates.append("gdpr_impact_details: populated")
            if not dry_run:
                existing.gdpr_impact_details = gdpr_considerations
...
def truly_comprehensive_sync(dry_run=True, entity_types=["all"]):
    ...
    try:
        ...
        print("Populating ALL available database fields from GitHub")
                ...
                if changed:
                    ...
                    epic_name = next(
                        (
                            ...
                            if v == parsed_labels.get("epic")
                        ),
                        "Unknown",
                    )
                    print(
                        f"  {us_id}: {', '.join(updates[:3])}{'...' if len(updates) > 3 else ''}"
                    )
                else:
