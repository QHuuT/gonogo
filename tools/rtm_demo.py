...
from src.be.models.traceability import Defect, Epic, Test, UserStory
...
class RTMDemoDataGenerator:
    ...
    def create_demo_epics(self) -> List[Epic]:
        ...
        for epic_data in demo_epics:
            ...
            existing = (
                self.db.query(Epic).filter(Epic.epic_id == epic_data["epic_id"]).first()
            )
            if not existing:
                epic = Epic(**epic_data)
                self.db.add(epic)
    ...
    def create_demo_user_stories(self, epics: List[Epic]) -> List[UserStory]:
        ...
        for story_data in demo_stories:
            existing = (
                ...
                .filter(UserStory.user_story_id == story_data["user_story_id"])
                .first()
            )
            if not existing:
    ...
    def create_demo_defects(self, epics: List[Epic]) -> List[Defect]:
        ...
        for defect_data in demo_defects:
            existing = (
                ...
                .filter(Defect.defect_id == defect_data["defect_id"])
                .first()
            )
            if not existing:
    ...
    def reset_to_demo_only(self):
        ...
        self.db.query(UserStory).filter(
            ~UserStory.user_story_id.like("US-DEMO-%")
        ).delete()
...
def generate_demo_report(output_path: str = None):
    ...
    try:
        ...
        html_content = generator.generate_html_matrix(
            {"include_tests": True, "include_defects": True}
        )
    finally:
        db.close()
...
def main():
    ...
    parser.add_argument("--html", action="store_true", help="Generate demo HTML report")
    parser.add_argument(
        "--populate-test-data",
        action="store_true",
    ...
    parser.add_argument(
        "--reset-to-demo", action="store_true", help="Reset database to demo-only state"
    )
