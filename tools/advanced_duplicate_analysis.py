...
def analyze_remaining_excess():
    ...
    try:
        ...
        print(f"Expected actual tests: 377")
        ...
        ).group_by(Test.test_type).order_by(func.count(Test.id).desc()).all()
        ...
        epic_patterns = session.query(
            Test.epic_id,
            func.count(Test.id).label('count')
        ).group_by(Test.epic_id).order_by(func.count(Test.id).desc()).all()
        ...
        bdd_step_tests = session.query(Test).filter(
            Test.test_file_path.like('%step_definitions%')
        ).count()
        ...
        for generic in generic_names:
            count = session.query(Test).filter(
                Test.test_function_name.like(f'%{generic}%')
            ).count()
        ...
        component_counts = session.query(
            Test.component,
            func.count(Test.id).label('count')
        ).group_by(Test.component).order_by(func.count(Test.id).desc()).all()
        ...
        suspicious_names = session.query(
            Test.test_function_name,
            func.count(distinct(Test.test_file_path)).label('file_count'),
            func.count(Test.id).label('total_count')
        ).filter(Test.test_function_name.isnot(None)).group_by(Test.test_function_name).having(
            func.count(distinct(Test.test_file_path)) > 3
        ).order_by(func.count(Test.id).desc()).limit(10).all()
...
def suggest_cleanup_strategies(analysis):
    ...
    print(f"1. Focus on BDD step definitions cleanup first")
    print(f"2. Implement file content verification")
    print(f"3. Remove malformed entries with NULL critical fields")
