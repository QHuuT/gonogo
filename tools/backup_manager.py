...
def backup(ctx, destinations: tuple, encrypt: bool, dry_run: bool):
    ...
    try:
        ...
        click.echo(
            f"Successful destinations: {result['successful_destinations']}/{result['total_destinations']}"
        )
        ...
        if verbose:
            ...
            for backup_result in result["backup_results"]:
                if backup_result.get("success", False):
                    ...
                    click.echo(f"  ✗ {backup_result['destination']}")
                    click.echo(
                        f"    Error: {backup_result.get('error', 'Unknown error')}"
                    )
...
@click.option(
    "--target", "-t", help="Target database path (defaults to current DATABASE_URL)"
)
@click.option("--force", is_flag=True, help="Force restoration without confirmation")
@click.option(
    "--verify-only", is_flag=True, help="Only verify backup without restoring"
)
@click.pass_context
def restore(
    ctx, backup_file: str, target: Optional[str], force: bool, verify_only: bool
):
    """
    ...
    try:
        ...
        if verbose:
            ...
            click.echo(
                f"  Integrity verified: {'✓' if result['integrity_verified'] else '✗'}"
            )
...
def status(ctx, json_output: bool, check_health: bool):
    ...
    try:
        ...
        click.echo(
            f"GDPR Compliance: {'✓ Enabled' if status_data['gdpr_compliance'] else '✗ Disabled'}"
        )
        click.echo(
            f"Encryption: {'✓ Enabled' if status_data['encryption_enabled'] else '✗ Disabled'}"
        )

        if check_health:
            health = status_data["database_health"]
            click.echo(
                f"Database Health: {'✓ Healthy' if health['status'] == 'healthy' else '✗ Unhealthy'}"
            )
            if health["status"] != "healthy":
...
@click.option(
    "--dry-run", is_flag=True, help="Show what would be cleaned up without deleting"
)
...
def cleanup(ctx, days: int, dry_run: bool, force: bool):
    ...
    try:
        ...
        for backup in old_backups:
            click.echo(
                f"  - {backup['file'].name} ({backup['age_days']} days old, {backup['size_mb']:.1f} MB)"
            )
        ...
        click.echo(
            f"Cleanup completed: {deleted_count}/{len(old_backups)} backups deleted"
        )
...
@cli.command()
@click.option(
    "--backup-file", "-f", required=True, help="Path to backup file to validate"
)
