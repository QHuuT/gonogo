...
def validate_template(template_path):
    ...
    if not isinstance(template["body"], list):
        print("  ERROR: 'body' must be a list")
        return False
    ...
    if template_name in required_inputs:
        ...
        if missing_inputs:
            print(
                f"  ERROR: Missing required input fields: {', '.join(missing_inputs)}"
            )
            return False

    print("  OK: Valid template")
    return True
...
def validate_labels(labels_path):
    ...
    if not isinstance(labels, list):
        print("  ERROR: Labels file must contain a list")
        return False
