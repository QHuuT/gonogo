...
class TestRTMBrowserClicks:
    ...
    def test_click_filter_button_and_check_behavior(self, driver):
        ...
        defect_buttons = driver.find_elements(
            By.XPATH, "//button[contains(@onclick, 'filterDefects')]"
        )
        ...
        epic_headers = driver.find_elements(
            By.XPATH, "//header[contains(@onclick, 'toggleEpicDetails')]"
        )
        if not epic_headers:
        ...
        print(
            f"Clicking defect filter button: '{button_text}' with onclick='{onclick_attr}'"
        )
        ...
        try:
            ...
            new_js_errors = [
                log
                for log in browser_logs_after
                if log["level"] == "SEVERE" and log not in browser_logs
            ]
            if new_js_errors:
            ...
            if new_url == original_url:
                pytest.fail(
                    "URL did not change after clicking filter button - filtering not working!"
                )

            # Check if filter parameters are in the URL
            if "defect_" not in new_url:
                pytest.fail(
                    "No defect filter parameters in URL after clicking defect filter button!"
                )

            print(
                "OK Filter button click resulted in URL change with filter parameters"
            )
            ...
            js_errors_click = [
                log for log in browser_logs_error if log["level"] == "SEVERE"
            ]
            if js_errors_click:
    ...
    def test_epic_toggle_still_works_after_filter(self, driver):
        ...
        try:
            ...
            if "defect_priority_filter=critical" not in current_url:
                pytest.fail(
                    "Filter was lost when clicking epic header - epic toggle broke filtering!"
                )
    ...
    def test_javascript_console_for_errors(self, driver):
        ...
        if js_errors:
            ...
            pytest.fail(
                f"JavaScript errors present: {[e['message'] for e in js_errors]}"
            )
    ...
    def test_filter_functions_are_defined(self, driver):
        ...
        for func_name in functions_to_test:
            try:
                ...
                if result != "function":
                    pytest.fail(
                        f"Function {func_name} is not defined or not a function (type: {result})"
                    )
