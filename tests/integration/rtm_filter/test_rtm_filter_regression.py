...
class TestRTMFilterRegression:
    ...
    def test_filter_buttons_generate_correct_onclick_handlers(self):
        ...
        defects_filter_pattern = (
            r'onclick="filterDefects\(\'([^\']+)\',\s*\'([^\']+)\',\s*\'([^\']+)\'\)"'
        )
        ...
        print(
            f"OK Found defect filter buttons with 3 parameters: {len(defects_matches)} buttons"
        )
    ...
    def test_javascript_file_loads_correctly(self):
        ...
        defects_func_pattern = (
            r"function filterDefects\s*\(\s*(\w+)\s*,\s*(\w+)\s*,\s*(\w+)\s*\)"
        )
    ...
    def test_status_filter_backend_functionality(self):
        ...
        response = requests.get(
            f"{self.RTM_URL}?format=html&us_status_filter=in_progress"
        )
        assert response.status_code == 200
