...
class TestRTMFilterIntegration:
    ...
    def test_rtm_html_structure_for_filtering(self):
        ...
        assert 'aria-label="User Stories for EP-' in html_content, "User Stories table aria-label not found"
        assert 'class="us-row"' in html_content, "User story rows not found"
        assert 'data-us-status=' in html_content, "User story status data attributes not found"

        # Test 3: Verify test table structure
        assert 'aria-label="Test Traceability for EP-' in html_content, "Test table aria-label not found"
        ...
        assert 'class="defect-filter-section"' in html_content, "Defect filter section not found"
        assert 'class="defect-row"' in html_content, "Defect rows not found"
        assert 'data-defect-priority=' in html_content, "Defect priority data attributes not found"
        assert 'data-defect-status=' in html_content, "Defect status data attributes not found"

        # Test 5: Verify filter buttons have correct attributes
        assert 'data-filter-group="epic-' in html_content, "Filter group data attributes not found"
        assert 'onclick="filterTestsByType(' in html_content, "Test filter onclick handlers not found"
        assert 'onclick="filterUserStoriesByStatus(' in html_content, "User story filter onclick handlers not found"
        assert 'onclick="filterDefects(' in html_content, "Defect filter onclick handlers not found"

        # Test 6: Verify JavaScript file is referenced
        assert 'rtm-interactions.js' in html_content, "RTM interactions JavaScript not referenced"
    ...
    def test_javascript_functions_are_exposed(self):
        ...
        assert 'window.filterUserStoriesByStatus = filterUserStoriesByStatus;' in js_content
    ...
    def test_filter_button_data_attributes_consistency(self):
        ...
        for pattern in us_filter_patterns:
            assert pattern in html_content, f"User story filter pattern not found: {pattern}"
        ...
        for pattern in defect_filter_patterns:
            assert pattern in html_content, f"Defect filter pattern not found: {pattern}"
    ...
    def test_javascript_selector_compatibility(self):
        ...
        assert '#epic-${epicId}' in js_content, "Epic container selector not in JavaScript"

        # Test 2: Test table selectors
        assert 'aria-label="Test Traceability' in html_content, "Test table aria-label not in HTML"
        assert 'table[aria-label*="Test Traceability"]' in js_content, "Test table selector not in JavaScript"

        # Test 3: User stories table selectors
        assert 'aria-label="User Stories' in html_content, "User stories table aria-label not in HTML"
        assert 'table[aria-label*="User Stories"]' in js_content, "User stories table selector not in JavaScript"

        # Test 4: Defect filter section selectors
        assert 'class="defect-filter-section"' in html_content, "Defect filter section not in HTML"
        assert '.defect-filter-section' in js_content, "Defect filter section selector not in JavaScript"
    ...
    def test_filter_count_display_elements(self):
        ...
        assert 'class="test-count-display"' in html_content or 'test-count-display' in html_content, "Test count display not found"
        assert 'class="us-count-display"' in html_content or 'us-count-display' in html_content, "User story count display not found"
        assert 'class="defect-count-display"' in html_content or 'defect-count-display' in html_content, "Defect count display not found"
    ...
    def test_animation_classes_and_functionality(self):
        ...
        assert 'function animateVisibility(' in js_content, "animateVisibility function not found"
    ...
    def test_accessibility_features(self):
        ...
        assert 'setAttribute(\'aria-pressed\'' in js_content, "ARIA pressed handling not in JavaScript"
        assert 'announceToScreenReader(' in js_content, "Screen reader announcements not found"
    ...
    def test_multiple_epic_independence(self):
        ...
        for epic_id in epic_containers:
            ...
            assert epic_specific_content in html_content, f"Epic-specific container not found for {epic_id}"
...
class TestRTMFilterRegression:
    ...
    def test_epic_expand_collapse_still_works(self):
        ...
        assert 'function toggleEpicDetails(' in js_content, "toggleEpicDetails function not found"
        assert 'window.toggleEpicDetails = toggleEpicDetails;' in js_content, "toggleEpicDetails not exposed globally"
    ...
    def test_search_functionality_preserved(self):
        ...
        assert 'function performSearch(' in js_content, "performSearch function not found"
