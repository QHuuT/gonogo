...
print('\n')
result = conn.execute(text(
    'SELECT COUNT(*) FROM tests WHERE test_function_name IS NOT NULL'
))
...
print(
    f'Tests with function names: {total_with_func} '
    f'({total_with_func*100//total}%)'
)
print(
    f'Tests without function names: {total - total_with_func} (BDD scenarios)'
)
