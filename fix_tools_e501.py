...
def get_e501_violations(file_path):
    ...
    try:
        result = subprocess.run(
            ['python', '-m', 'flake8', '--select=E501', '--show-source', str(file_path)],
                    ...
                    if 'print(f"' in line:
                        ...
                        if match:
                            ...
                            if len(msg) > 60:
                                ...
                                split_points = [m.start() for m in re.finditer(r'[, ]', msg)]
                                mid_point = len(msg) // 2
...
def main():
    ...
    for py_file in python_files:
        if py_file.name in ['add_epic_metrics_columns.py', 'add_test_markers.py',
                           'update_user_story_from_github.py', 'validate_test_associations.py']:
        ...
        try:
            subprocess.run(['python', '-m', 'py_compile', str(py_file)],
                         check=True, capture_output=True, cwd='tools')
        ...
        if apply_common_e501_fixes(py_file):
            ...
            if len(violations_after) < len(violations_before):
                print(f"  FIXED: {len(violations_before) - len(violations_after)} violations")
                fixed += 1
            else:
