...
from .log_failure_correlator import LogFailureCorrelator, FailureContext
...
class TestFailureIssueCreator:
    ...
    def create_issue_from_failure(self,
                                failure_id: int,
    ...
    def create_batch_issues_from_failures(self,
                                        failure_ids: List[int],
                                        auto_assign: bool = True,
                                        dry_run: bool = False) -> List[IssueCreationResult]:
        """
        ...
        for failure_id in failure_ids:
            try:
                ...
                results.append(IssueCreationResult(
                    success=False,
                    issue_number=None,
                    issue_url=None,
    ...
    def get_recent_failure_candidates(self, days: int = 7,
    ...
    def _generate_issue_body(self, context: FailureContext) -> str:
        ...
        if context.related_failures:
            ...
            for related in context.related_failures[:5]:  # Limit to top 5
                body_parts.append(f"- **{related.get('test_name', 'Unknown')}**: {related.get('failure_message', 'No message')[:100]}...")
    ...
    def generate_batch_creation_report(self, results: List[IssueCreationResult]) -> str:
        ...
        if successful:
            ...
            for result in successful:
                report_lines.append(f"- [#{result.issue_number}]({result.issue_url}) - Labels: {', '.join(result.labels_applied)}")
