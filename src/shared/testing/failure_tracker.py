...
class FailureTracker:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if existing:
                ...
                conn.execute("""
                    ...
                    WHERE id = ?
                """, (new_count, failure.last_seen.isoformat(),
                     failure.session_id, failure.execution_mode, failure_id))
                return failure_id
            else:
                ...
                """, (
                    failure.test_id, failure.test_name, failure.test_file,
                    failure.failure_message, failure.stack_trace,
                    failure.category.value, failure.severity.value,
                    failure.error_hash, failure.first_seen.isoformat(),
                    failure.last_seen.isoformat(), failure.occurrence_count,
                    failure.environment_info, failure.coverage_info,
                    failure.execution_mode, failure.session_id,
    ...
    def categorize_failure(self, error_message: str, stack_trace: str) -> FailureCategory:
        ...
        patterns = {
            ...
            FailureCategory.NETWORK_ERROR: [
                "connectionerror", "network", "socket", "dns"
            ],
            FailureCategory.DATABASE_ERROR: [
                "database", "sql", "sqlite", "postgresql", "connection refused"
            ],
            ...
            FailureCategory.DEPENDENCY_ERROR: [
                "dependency", "version", "compatibility", "requirement"
            ]
        }
    ...
    def _calculate_trend_direction(self, weekly_data: List[Tuple]) -> str:
        ...
        earlier_avg = sum(w[1] for w in weekly_data[:-2]) / len(weekly_data[:-2]) if len(weekly_data) > 2 else weekly_data[0][1]
    ...
    def cleanup_old_failures(self, days: int = 90):
        ...
        with sqlite3.connect(self.db_path) as conn:
            deleted_count = conn.execute(
                "DELETE FROM test_failures WHERE last_seen < ?",
                (cutoff_date,)
            ).rowcount
            conn.execute(
                "DELETE FROM failure_patterns WHERE last_occurrence < ?",
                (cutoff_date,)
            )
