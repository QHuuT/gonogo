...
class DefectValidationService:
    ...
    def validate_all_relationships(self) -> Dict[str, any]:
        ...
        try:
            ...
            linked_defects = self.session.query(Defect).filter(
            ...
            orphaned_defects = self.session.query(Defect).filter(
            ...
            # Log summary
            logger.info(f"Validation complete: {results['valid_links']} valid, "
                       f"{results['broken_links']} broken, {results['orphaned_defects']} orphaned")
    ...
    def validate_defect_relationship(self, defect_id: str) -> Dict[str, any]:
        ...
        try:
            defect = self.session.query(Defect).filter(
    ...
    def get_relationship_health_metrics(self) -> Dict[str, any]:
        ...
        try:
            ...
            for defect in self.session.query(Defect).filter(
                Defect.github_user_story_number.isnot(None)
            ).all():
                user_story = self.session.query(UserStory).filter(
                    UserStory.github_issue_number == defect.github_user_story_number
            ...
            return {
                ...
                'relationship_health_score': round(health_score, 2),
                'linked_percentage': round(linked_percentage, 2),
                'valid_link_percentage': round(valid_link_percentage, 2),
                'broken_link_percentage': round(broken_link_percentage, 2),
