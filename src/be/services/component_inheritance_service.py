...
class ComponentInheritanceService:
    ...
    def inherit_defect_component(self, defect: Defect, force: bool = False) -> bool:
        ...
        logger.info(
            (
                f"Inherited component for {defect.defect_id}: {old_component} → {defect.component}"
            )
        )
        return True
    ...
    def inherit_test_component(self, test: Test, force: bool = False) -> bool:
        ...
        if not user_story:
            logger.warning(
                (
                    f"Test {test.id} references non-existent User Story #{test.github_user_story_number}"
                )
            )
            return False
