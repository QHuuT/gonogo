...
async def create_dependency(dependency: DependencyCreate, db: Session = Depends(get_db)):
    ...
    if cycles:
        ...
        raise HTTPException(
            status_code=400, detail=(f"Creating this dependency would create a cycle: {cycle_description}")
        )
...
async def detect_cycles(db: Session = Depends(get_db)):
    ...
    return {"cycles_detected": len(cycles) > 0, "cycle_count": len(cycles), "cycles": cycles}


@router.get("/analysis/critical-path", response_model=CriticalPathResponse)
async def calculate_critical_path(
    epic_ids: Optional[str] = Query(None, description="IDs des Epics séparés par virgule"),
    db: Session = Depends(get_db),
):
...
async def get_dependency_statistics(db: Session = Depends(get_db)):
    ...
    for dep_type in DependencyType:
        count = (
            db.query(EpicDependency)
            .filter(EpicDependency.dependency_type == dep_type.value, EpicDependency.is_active == True)
            .count()
        )
    ...
    for priority in ["critical", "high", "medium", "low"]:
        count = (
            db.query(EpicDependency)
            .filter(EpicDependency.priority == priority, EpicDependency.is_active == True)
            .count()
        )
